...
const mocks = vi.hoisted(() => ({
  addOperation: vi.fn(),
  createOperation: vi.fn((type: string, amountCents: number, date: string, comment: string) => ({
    type,
    amountCents,
    date,
    comment,
...
describe('OperationsPage', () => {
  ...
  it.each(['', '-10', 'hello', '10.999'])('rejects the invalid amount %s', async (amount) => {
    ...
    expect((await screen.findByRole('alert')).textContent).toContain(
      'Veuillez renseigner un montant positif valide et une date.',
    );
  ...
  it('explains when no deposit or withdrawal is available', () => {
    ...
    expect(
      screen.getByRole('heading', { name: 'Aucun dépôt ou retrait enregistré' }),
    ).not.toBeNull();
    expect(
      screen.getByText('Vos prochains mouvements financiers apparaîtront ici.'),
    ).not.toBeNull();
