...
describe('WinamaxImportPage', () => {
  beforeEach(() => {
    mocks.getConfiguration.mockResolvedValue(undefined);
  ...
  it('shows the Winamax folder import as the only import method', async () => {
  ...
  it('disables folder selection in read-only mode', () => {
    render(<WinamaxImportPage onImported={onImported} readOnly />);

    const button = screen.getByRole('button', { name: 'Choisir le dossier Winamax' }) as HTMLButtonElement;
    expect(button.disabled).toBe(true);
  });

  it('shows the compatibility explanation without a Chromium directory picker', () => {
    Reflect.deleteProperty(window, 'showDirectoryPicker');

    render(<WinamaxImportPage onImported={onImported} />);

    expect(screen.getByText(/navigateur compatible Chromium récent/i)).not.toBeNull();
    expect(screen.queryByRole('button', { name: 'Choisir le dossier Winamax' })).toBeNull();
  });
});
