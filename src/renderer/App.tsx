...
function AccessBanner({ access }: { access: AccessState }): ReactElement { if (access.status === 'active') return <></>; if (access.status === 'trial') return <section className="access-banner" role="status"><strong>Essai gratuit en cours</strong><span>Expire le {new Date(access.trialExpiresAt ?? '').toLocaleString('fr-FR')}.</span><button className="button-secondary" type="button">Acheter Bankroll Pilot</button></section>; if (access.status === 'expired') return <section className="access-banner expired" role="status"><strong>Votre période d’essai est terminée</strong><span>Vos données sont conservées. L’application est en lecture seule et l’export reste disponible.</span><button className="button-secondary" type="button">Acheter Bankroll Pilot</button><button className="text-button" type="button">J’ai déjà acheté</button></section>; return <section className="access-banner expired" role="alert"><strong>Service d’accès indisponible</strong><span>Vos données restent consultables. Les modifications sont désactivées.</span></section>; }
