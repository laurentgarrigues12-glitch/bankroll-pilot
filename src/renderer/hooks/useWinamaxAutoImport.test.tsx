...
describe('useWinamaxAutoImport', () => {
  ...
  it('reports when detected files are not a completed importable session yet', async () => {
    ...
    expect(result.current.message).toMatch(/session terminée/i);
    expect(onImported).not.toHaveBeenCalled();
  });

  it('reports permission errors instead of failing silently', async () => {
    mocks.scan.mockRejectedValue(new Error('L’autorisation de lecture du dossier Winamax a été refusée.'));
