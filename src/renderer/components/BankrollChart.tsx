...
function ChartTooltip({
  ...
  return (
    <div className="chart-tooltip">
      ...
      <strong>{currency.format(point.bankroll)}</strong>
...
export function BankrollChart({ data, state = 'ready' }: BankrollChartProps): ReactElement {
  if (state === 'loading')
    return (
      <section className="chart-state" aria-label="Chargement du graphique">
  ...
  if (state === 'error')
    return (
      <section className="chart-state error-state" role="alert">
        <AlertCircle size={24} />
        <p>Le graphique est indisponible pour le moment.</p>
      </section>
    );
  if (state === 'empty' || data.length === 0)
    return (
      <section className="chart-state chart-empty" aria-label="Aucune donnée">
        <span className="empty-state-icon">
          <BarChart3 size={22} />
        </span>
        <span className="state-badge">En attente de données</span>
        <h3>Votre courbe se dessinera ici</h3>
        <p>Importez des parties Winamax ou ajoutez une opération pour démarrer le suivi.</p>
      </section>
    );

  return (
    <section className="bankroll-chart" aria-label="Évolution de la bankroll">
      <ResponsiveContainer width="100%" height={300}>
        <LineChart data={data} margin={{ top: 12, right: 12, left: -10, bottom: 0 }}>
          ...
          <Tooltip
            ...
            cursor={{ stroke: 'var(--accent-secondary)', strokeDasharray: '3 4' }}
          />
          <Line
            type="monotone"
            dataKey="bankroll"
